 ...
 *
 * Build one deliberately with `SITE_CLIENT=<slug> pnpm build` — the FULL
 * chain, not `pnpm exec astro build`. That distinction cost a deploy: bare
 * `astro build` skips `gen-headers.mjs`, so the first publish of all five went
 * out with no `_headers` and therefore no CSP. The reason anybody reached for
 * the bare command was `check-fabrication.mjs`, which used to refuse a build
 * with no prospect record and take the eight later gates down with it; it now
 * scans a `portfolio-` build against an empty allowance list instead, so the
 * whole chain runs and the headers are generated by construction rather than
 * by remembering. See docs/evidence/fabrication-recordless-red.md.
 *
 * Then delete its `dist/<slug>/` before running a deploy.
 */
